...
//    var allies: List<Ally>
    ...
    fun playerAction(game: Game, room: Room) : Boolean  {
        ...
        when (game.gameStage) {
            ...
            2 -> {
                ...
                println("5. Посмотреть характеристику врагов")
            ...
            3 -> {
                println("Бой в кристальных пещерах продолжается. Выберите действие:")
                println("1. Атаковать")
                ...
                when (choice) {
                    ...
                    7 -> {
                        if (game.player.allies.isNotEmpty()) {
                            showAlliesStats(game)
                        } else {
                            println("У вас нет союзников.")
                        }
                    }
                    else -> {
                        println("Неверный выбор. Попробуйте снова.")
    ...
    private fun useSpells(game: Game, room: Room) {
        println("Выберите заклинание:")
        println("1. Исцеление (60 маны)")
        println("2. Огненный шар (40 маны)")
        println("3. Землятрясение (80 маны)")
        println("4. Тёмная Стрела (60 маны)")
        println("5. Ледяной шар (60 маны)")
        println("6. Мистическая Волна (60 маны)")
        println("7. Магия крови (100 маны)")

        val choice = readlnOrNull()?.toIntOrNull() ?: -1 // Обработка ввода пользователя
        ...
        when (choice) {
            1 -> {
                ...
                println("Выберите того, кого хотите исцелить:")
                println("1. Себя")
                println("2. Астрала")
                println("3. Союзника")

                val healChoice = readlnOrNull()?.toIntOrNull() ?: -1

                when (healChoice) {
                    1 -> {
                        game.player.mana -= manaCost
                        HealSpell().cast(game.player)
                        println("Использовано $manaCost маны. Осталось ${game.player.mana} маны.")
                    }

                    2 -> {
                        if (game.player.astral != null) {
                            game.player.mana -= manaCost
                            HealSpell().cast(game.player.astral!!)
                            println("Использовано $manaCost маны. Осталось ${game.player.mana} маны.")
                        } else {
                            println("У вас нет астрала.")
                        }
                    }

                    3 -> {
                        if (game.player.allies.isNotEmpty()) {
                            println("Выберите союзника:")
                            for (ally in game.player.allies) {
                                println("${game.player.allies.indexOf(ally) + 1}. ${ally.name}")
                            }
                            val allyChoice = readlnOrNull()?.toIntOrNull() ?: -1
                            if (allyChoice in 1..game.player.allies.size) {
                                game.player.mana -= manaCost
                                val ally = game.player.allies[allyChoice - 1]
                                HealSpell().cast(ally)
                                println("Использовано $manaCost маны. Осталось ${game.player.mana} маны.")
                            } else {
                                println("Неверный выбор. Попробуйте снова.")
            ...
            3 -> {
                ...
                game.player.mana -= manaCost
                EarthQuakeSpell().cast(room.enemies)
                println("Использовано $manaCost маны. Осталось ${game.player.mana} маны.")
            }
    ...
    private fun showAlliesStats(game: Game) {
        for (ally in game.player.allies) {
